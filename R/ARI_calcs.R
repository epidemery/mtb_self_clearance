...
load(here::here("results","All_3_ind_edited.Rdata"))
...
fyear <- start_year
tdz <- All$year-fyear
tez <- (start_year+1):end_year - fyear                  #extrapolation times
...
saveRDS(erw, file = here::here("results",paste0("ARI_GPR_JPN_regression_",file_suffix,".Rdata")))

runs <- mvrnorm(n=replicates,mu=as.numeric(tot$mg),Sigma=as.matrix(symmpart(tot$cg)))

runsdf <- data.frame(year=tez+fyear,iso3=as.character(unique(All$iso3)),
                     lari=c(t(runs)),replicate=rep(1:nrow(runs),each=ncol(runs)))

saveRDS(runsdf, file = here::here("results",paste0("ARI_GPR_JPN_replicates_", file_suffix, ".Rdata")))
...
saveRDS(erw, file = here::here("results", paste0("ARI_GPR_CHN_regression_",file_suffix,".Rdata")))
...
saveRDS(runsdf, file = here::here("results",paste0("ARI_GPR_CHN_replicates_", file_suffix, ".Rdata")))
...
saveRDS(erw, file = here::here("results", paste0("ARI_GPR_IND_regression_",file_suffix,".Rdata")))
...
saveRDS(runsdf, file = here::here("results",paste0("ARI_GPR_IND_replicates_", file_suffix, ".Rdata")))
