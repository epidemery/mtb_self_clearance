...
Nro <- read.csv(here::here("data",'inputData_GTB_estimate_ind_removed.csv'))
...
## Cauthen
C <- read.csv(here::here("data",'inputData_Cauthen_ARI.csv'))
C$ari <- 1e-2*C$ARI
## get ages
...
## new data ---
C2 <- read.csv(here::here("data",'inputData_Review_ARI.csv'))
C2$ari <- 1e-2*C2$ARI
...
K <- read.csv(here::here("data",'inputData_GTB_kidestimate.csv'))
...
## age split
load(here::here("results",'kidsplit.Rdata'))              #Dodd,Sismanidis,Seddon
AF[,pu5:=a/(a+b)]
...
save(All,file=here::here("results","All_3_ind_edited.Rdata"))
